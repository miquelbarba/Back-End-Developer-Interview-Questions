...
* Why are global and static objects evil? Can you show it with a code example?<br/>
  *Because the state is shared between all the parts of the program. So, the state can be changed in any place of the program, making hard to reason about the behaviour.*
* Tell me about Inversion of Control and how it improves the design of code.<br/>
  *Changes the flow of the program, instead of objects calling other objects, implement an object with a known interface that is called by a third party (framework), that "changes" the flow. The objective is to decouple the code*
